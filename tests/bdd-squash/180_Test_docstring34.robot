*** Settings ***
Documentation    Test_docstring34
Metadata         ID                           180
...
Test Setup
    ...
    ...                You can define the ${TEST_180_SETUP} variable with a keyword for setting up this specific test.
    ...                If both are defined, ${TEST_180_SETUP} will be run after ${TEST_SETUP}.

    ${TEST_SETUP_VALUE} =        Get Variable Value    ${TEST_SETUP}
    ${TEST_180_SETUP_VALUE} =    Get Variable Value    ${TEST_180_SETUP}
    IF    $TEST_SETUP_VALUE is not None        Run Keyword    ${TEST_SETUP}
    IF    $TEST_180_SETUP_VALUE is not None    Run Keyword    ${TEST_180_SETUP}

Test Teardown
    [Documentation]    test teardown
    ...                You can define the ${TEST_TEARDOWN} variable with a keyword for tearing down all your tests.
    ...                You can define the ${TEST_180_TEARDOWN} variable with a keyword for tearing down this specific test.
    ...                If both are defined, ${TEST_TEARDOWN} will be run after ${TEST_180_TEARDOWN}.

    ${TEST_180_TEARDOWN_VALUE} =    Get Variable Value    ${TEST_180_TEARDOWN}
    ${TEST_TEARDOWN_VALUE} =        Get Variable Value    ${TEST_TEARDOWN}
    IF    $TEST_180_TEARDOWN_VALUE is not None    Run Keyword    ${TEST_180_TEARDOWN}
