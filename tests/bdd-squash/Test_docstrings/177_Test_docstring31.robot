*** Settings ***
Documentation    Test_docstring31
Metadata         ID                           177
...
Test Setup
    ...
    ...                You can define the ${TEST_177_SETUP} variable with a keyword for setting up this specific test.
    ...                If both are defined, ${TEST_177_SETUP} will be run after ${TEST_SETUP}.

    ${TEST_SETUP_VALUE} =        Get Variable Value    ${TEST_SETUP}
    ${TEST_177_SETUP_VALUE} =    Get Variable Value    ${TEST_177_SETUP}
    IF    $TEST_SETUP_VALUE is not None        Run Keyword    ${TEST_SETUP}
    IF    $TEST_177_SETUP_VALUE is not None    Run Keyword    ${TEST_177_SETUP}

Test Teardown
    [Documentation]    test teardown
    ...                You can define the ${TEST_TEARDOWN} variable with a keyword for tearing down all your tests.
    ...                You can define the ${TEST_177_TEARDOWN} variable with a keyword for tearing down this specific test.
    ...                If both are defined, ${TEST_TEARDOWN} will be run after ${TEST_177_TEARDOWN}.

    ${TEST_177_TEARDOWN_VALUE} =    Get Variable Value    ${TEST_177_TEARDOWN}
    ${TEST_TEARDOWN_VALUE} =        Get Variable Value    ${TEST_TEARDOWN}
    IF    $TEST_177_TEARDOWN_VALUE is not None    Run Keyword    ${TEST_177_TEARDOWN}
